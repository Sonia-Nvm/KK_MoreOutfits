...
namespace KK_Plugins.MoreOutfits
{
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        private void Awake()
        {
            ...
            {
                Logger.LogInfo("    --> MORE OUTFITS <-- VR SCENE INIT");
                harmony.Patch(VRHSceneType.GetMethod("MapSameObjectDisable", AccessTools.all), new HarmonyMethod(typeof(Hooks).GetMethod(nameof(Hooks.VRHScene_MapSameObjectDisable), AccessTools.all)));
            }
        }
